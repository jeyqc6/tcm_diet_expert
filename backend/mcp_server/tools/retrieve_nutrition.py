...
# -*- coding: utf-8 -*-
"""
retrieve_nutrition MCP 工具：查 knowledge_chunks WHERE domain='nutrition' 的向量检索。
签名对齐 docs/ARCHITECTURE.md §2.2；混合检索(filters)的具体语义见
backend/mcp_server/tools/_retrieval_common.py 的模块文档。

调用方：Nutrition SubAgent(事实查询/候选评估/单领域/完整推荐分支均可能调用，§2.2)。

⚠️ ARCHITECTURE.md §1.1 提醒过：recipe_xiachufang.json 的样本 chunk 也挂在
nutrition domain 下（靠 source_type='recipe_json' 区分），检索结果里可能混着
菜谱 chunk，必要时用 filters={"source_type": [...]} 排除。

状态：检索逻辑本身可用；MCP 协议层的权限声明(§2.3)还没做，见 backend/mcp_server/server.py。
